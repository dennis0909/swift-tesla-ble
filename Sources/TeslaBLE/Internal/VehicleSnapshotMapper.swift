...
//
// This is the ONLY file inside Sources/ that sees CarServer_* protobuf types.
// All other layers work with Swift-native DTOs.
...
enum VehicleSnapshotMapper {
    ...
    static func map(_ data: CarServer_VehicleData) -> TeslaVehicleSnapshot {
        TeslaVehicleSnapshot(
            ...
            chargeSchedule: data.hasChargeScheduleState ? ChargeScheduleStateDTO() : nil,
            preconditionSchedule: data.hasPreconditioningScheduleState ? PreconditionScheduleStateDTO() : nil,
            parentalControls: data.hasParentalControlsState ? mapParentalControls(data.parentalControlsState) : nil,
        )
    }

    /// Fast-path: returns only the drive state DTO (e.g. for 2-second speed polls).
    static func mapDrive(_ data: CarServer_VehicleData) -> DriveStateDTO {
        guard data.hasDriveState else { return DriveStateDTO() }
        return mapDrive(data.driveState)
    }

    // MARK: - Sub-state mappers

    private static func mapCharge(_ pb: CarServer_ChargeState) -> ChargeStateDTO {
        ChargeStateDTO(
    ...
    private static func mapClimate(_ pb: CarServer_ClimateState) -> ClimateStateDTO {
        ClimateStateDTO(
    ...
    private static func mapDrive(_ pb: CarServer_DriveState) -> DriveStateDTO {
    ...
    private static func mapClosures(_ pb: CarServer_ClosuresState) -> ClosuresStateDTO {
        let sunroofState: ClosuresStateDTO.SunroofState? = pb.hasSunRoofState
    ...
    private static func mapTirePressure(_ pb: CarServer_TirePressureState) -> TirePressureStateDTO {
        TirePressureStateDTO(
            frontLeft: TirePressureStateDTO.Tire(
            ...
            frontRight: TirePressureStateDTO.Tire(
            ...
            rearLeft: TirePressureStateDTO.Tire(
            ...
            rearRight: TirePressureStateDTO.Tire(
    ...
    private static func mapMedia(_ pb: CarServer_MediaState) -> MediaStateDTO {
        MediaStateDTO(
    ...
    private static func mapMediaDetail(_ pb: CarServer_MediaDetailState) -> MediaDetailStateDTO {
        MediaDetailStateDTO(
    ...
    private static func mapSoftwareUpdate(_ pb: CarServer_SoftwareUpdateState) -> SoftwareUpdateStateDTO {
        SoftwareUpdateStateDTO(
    ...
    private static func mapParentalControls(_ pb: CarServer_ParentalControlsState) -> ParentalControlsStateDTO {
        ParentalControlsStateDTO(
    ...
    private static func mapShift(_ pb: CarServer_ShiftState) -> DriveStateDTO.ShiftState? {
        guard let type = pb.type else { return nil }
        switch type {
        case .p: return .park
    ...
    private static func mapSeatHeater(_ rawLevel: Int32) -> ClimateStateDTO.SeatHeaterLevel? {
        ClimateStateDTO.SeatHeaterLevel(rawValue: Int(rawLevel))
    }
}
